...
def view_all(request):
    if not request.user.is_authenticated():
        return redirect(reverse('user:login'))
    requests = Request.objects.filter(~Q(user=request.user)).order_by('user__plan')
...
def view_connections(request):
    ...
    requests = Request.objects.filter(user__in=connections, anon=False).order_by('user__plan')
    paginator = Paginator(requests, 25) # Show 25 contacts per page
    page = request.GET.get('page')
    try:
        requests = paginator.page(page)
